...
on how you're going to solve a particular (business) problem.

When moving our code to production however, we want to have more guarantees about the behaviour of
our code. Writing (unit) tests is one way to get more guarantees, but we also make heavy use of
...
inputs, can I _loosen up_ those parameters to make the function more universally useful, the way
duck typing does?
...
`Protocol` from the `typing` module lets us do that. Just like with duck typing, **Protocols** lets
...
`measurements` but it does know what those items have a `timestamp` field because they adhere to
...
Instead of having to specify the behaviour and properties both on a type and on the functions that
use it, we only have to specify it on a function, without caring about the types of the objects
that are provided to the function.

## Protocol and TypeVar

You can also use Protocols together with `TypeVar` for even more general functions that are still
type checked. One use-case that comes to mind, is when you don't care about the input type to a
function, as long as it follows a protocol, but you also want to guarantee that the _output_ of
the function is of the same type as the _input_, no matter what the exact type is.
...
def measurement_at_timezone(measurement: M, tz: tzinfo) -> M:
    measurement.timestamp = measurement.timestamp.astimezone(tz)
    return measurement
```

Here we created a function that takes any object that has a `timestamp` field and guarantees that
