...
gave discussed the drawbacks of using an IDE for programming. Needless to say, when you touch
...
I think it's good form to only voice an opinion only when we can see the whole picture. I also
...
- Everyone should be able to build and run a project, regardless of the choice of editor,
  preferably through the terminal. This means that we'll always prefer external tools for this as
  well. Many languages provide tooling for this:
  - Java → Maven/Gradle
  - JS/TS → NPM
  - Go → Go
  - Rust → Cargo
...
[my dotfiles](https://github.com/DandyDev/dotfiles) as well.
...
Sometimes, but rarely, I use VSCodium for editing or viewing files which I want to reformat/reindent
which doesn't work well for me in Sublime Text. For example: viewing/editing minified JSON files.
